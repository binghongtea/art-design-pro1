<template>
    <el-dialog title="设备详情" v-model="showDialog" width="600px">
        <el-form ref="formRef" :model="formData" label-width="80px" :rules="rules">
            ...
            <el-form-item label="设备类型" prop="deviceType">
                <el-select v-model="formData.deviceType" placeholder="请选择" disabled>
...
import { UpdatePoint } from '@/api/equip';
...
const handleOk = () => {
    formRef.value.validate((valid: boolean) => {
        if (valid) {
            UpdatePoint(formData.value).then((res: any) => {
                props.refreshList()
                closeDialog()
...
onMounted(() => {
    formData.value = {
        spaceId: props.skipData.spaceId,
        pointId: props.skipData.pointId,
        x: props.skipData.x,
        y: props.skipData.y,
        pointName: props.skipData.pointName,
        deviceCode: props.skipData.deviceCode,
        deviceType: props.skipData.deviceType,
    }
